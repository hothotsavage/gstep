...
func GetTemplate(templateId int, versionId int, tx *gorm.DB) *entity.Template {
	var entities []*entity.Template
	err := tx.Where("template_id=? and version=?", templateId, versionId).Order("version desc").Find(&entities).Error
	if nil != err {
		panic(err)
	}
